## Frontend: Structured Generation Language (Coalinfer)
...
### Quick Start
The example below shows how to use coalinfer to answer a mulit-turn question.
...
```python
from coalinfer import function, system, user, assistant, gen, set_default_backend, RuntimeEndpoint
...
```python
from coalinfer import function, system, user, assistant, gen, set_default_backend, OpenAI
...
### Language Feature
To begin with, import coalinfer.
```python
import coalinfer as sgl
```

`coalinfer` provides some simple primitives such as `gen`, `select`, `fork`, `image`.
