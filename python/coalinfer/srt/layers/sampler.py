...
from coalinfer.srt.managers.schedule_batch import global_server_args_dict
from coalinfer.srt.sampling.sampling_batch_info import SamplingBatchInfo
...
class Sampler(CustomOp):
    ...
    def forward_cuda(
        ...
        if not global_server_args_dict["disable_flashinfer_sampling"]:
            ...
            probs = flashinfer_top_k_renorm(probs, sampling_info.top_ks)
            probs = flashinfer_top_p_renorm(probs, sampling_info.top_ps)
            batch_next_token_ids, success = flashinfer_min_p(
...
@torch_custom_op("my_lib::flashinfer_top_k_renorm", mutates_args={})
def flashinfer_top_k_renorm(
    probs: torch.Tensor,
    top_ks: torch.Tensor,
) -> torch.Tensor:
    return top_k_renorm_prob(probs, top_ks)

@torch_custom_op("my_lib::flashinfer_top_p_renorm", mutates_args={})
def flashinfer_top_p_renorm(
    probs: torch.Tensor,
    top_ps: torch.Tensor,
) -> torch.Tensor:
    return top_p_renorm_prob(probs, top_ps)

@torch_custom_op("my_lib::flashinfer_min_p", mutates_args={})
def flashinfer_min_p(
    probs: torch.Tensor,
    uniform_samples: torch.Tensor,
    min_ps: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    return min_p_sampling_from_probs(probs, uniform_samples, min_ps)
...
@flashinfer_top_k_renorm.register_fake
def _1(
    probs: torch.Tensor,
    top_ks: torch.Tensor,
) -> torch.Tensor:
    ...
    return torch.zeros(bs, dtype=torch.int32, device=probs.device)

@flashinfer_top_p_renorm.register_fake
def _2(
    probs: torch.Tensor,
    top_ps: torch.Tensor,
) -> torch.Tensor:
...
@flashinfer_min_p.register_fake
def _(
    probs: torch.Tensor,
    uniform_samples: torch.Tensor,
    min_ps: torch.Tensor,
