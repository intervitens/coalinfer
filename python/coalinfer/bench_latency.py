...
# Usage (latency test)
## with dummy weights:
python -m coalinfer.bench_latency --model-path meta-llama/Meta-Llama-3-8B-Instruct --load-format dummy
## sweep through multiple data points and store (append) the results in a jsonl file:
python -m coalinfer.bench_latency --model-path meta-llama/Meta-Llama-3-8B-Instruct --batch 1 12 14 --input-len 256 512 --output-len 32 256 --result-filename out.jsonl
## do some changes, and store the results under a different run_name:
python -m coalinfer.bench_latency --model-path meta-llama/Meta-Llama-3-8B-Instruct --batch 1 12 14 --input-len 256 512 --output-len 32 256 --result-filename out.jsonl --run-name after
## plot the results in series of lines:
python -m coalinfer.bench_latency --result-filename out.jsonl --graph-sql="select run_name, batch_size, prefill_throughput from results"

# Usage (correctness test):
python -m coalinfer.bench_latency --model-path TinyLlama/TinyLlama-1.1B-Chat-v0.4 --correct
...
from coalinfer.srt.hf_transformers_utils import get_tokenizer
from coalinfer.srt.managers.schedule_batch import Req, ScheduleBatch
from coalinfer.srt.model_config import ModelConfig
from coalinfer.srt.model_executor.model_runner import ModelRunner
from coalinfer.srt.sampling.sampling_params import SamplingParams
from coalinfer.srt.server_args import ServerArgs
from coalinfer.srt.utils import suppress_other_loggers
